...
- **Per upload:** download watermark-free → verify audio → blur + light de-dup edit
  ...
  upload as public Short → record in `data/channel_4.db` (committed back) → Discord ping

## Layout

```
channels.yaml            THE config (schema: Automation.md §5)
run.py                   entry: python run.py --slot N --channel channel_4 [--dry-run]
...
|---|---|---|
| `CHANNEL_4_CLIENT_SECRET` | base64 of `credentials/channel_4_client_secret.json` | upload |
| `CHANNEL_4_TOKEN` | base64 of `tokens/channel_4_token.json` | upload |
...
python run.py --slot 1 --channel channel_4 --dry-run
```
